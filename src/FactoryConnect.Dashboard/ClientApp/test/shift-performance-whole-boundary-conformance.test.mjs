...
function metricStates(shift) {
  return [
    shift.availability.state,
    shift.utilization.state,
...
test("whole boundary preserves configured population, ordering, and all lineage states", () => {
  ...
  const m1Shift2 = report(m1, "shift-b", "2026-09-02T08:00:00Z", "2026-09-02T16:00:00Z", [
  ...
  const projectedM1 = overview.groups[0].machines[0];
  const projectedM2 = overview.groups[0].machines[1];
  const projectedM3 = overview.groups[1].machines[0];

  assert.deepEqual(projectedM1.shifts.map(shift => shift.shift.shiftId), ["shift-a", "shift-b"]);
  assert.deepEqual(projectedM2.shifts, []);

  assert.equal(projectedM1.shifts[0].sourceRevision, null);
  assert.deepEqual(metricStates(projectedM1.shifts[0]), ["missing", "missing", "missing", "missing", "missing"]);

  assert.equal(projectedM3.shifts[0].sourceRevision, revision);
  assert.equal(projectedM3.shifts[0].shift, m3Shift.shift);
  assert.equal(projectedM3.shifts[0].productionLineId, "line-b");
  assert.deepEqual(metricStates(projectedM3.shifts[0]), ["missing", "missing", "missing", "missing", "missing"]);

  assert.equal(projectedM1.shifts[1].availability.value, "0.80");
  assert.equal(projectedM1.shifts[1].performance.value, "0.50");
  assert.equal(projectedM1.shifts[1].quality.value, "0.90");
  assert.equal(projectedM1.shifts[1].oee.value, "0.37");
  assert.equal(projectedM1.shifts[1].utilization.state, "missing");
});
...
test("whole boundary preserves authoritative evaluation states without normalization", () => {
  ...
  const item = report(m1, "shift-a", "2026-09-02T00:00:00Z", "2026-09-02T08:00:00Z", [
    metric("Availability", "calculated", { value: "0.8000" }),
    metric("Utilization", "unavailable", { reasonCode: "missing-power-on", reasonOperandName: "PowerOn" }),
    metric("Performance", "insufficient-evidence", { reasonCode: "missing-reference-time", reasonOperandName: "ReferenceTime" }),
  ]);

  const shift = mapShiftPerformanceOverview(day, [m1], { items: [item] }).groups[0].machines[0].shifts[0];

  assert.deepEqual(shift.availability, {
  ...
  assert.deepEqual(shift.utilization, {
  ...
  assert.deepEqual(shift.performance, {
  ...
  assert.equal(shift.quality.state, "missing");
  assert.equal(shift.oee.state, "missing");
});
...
test("whole boundary rejects malformed authority and publishes no partial model", () => {
  ...
  const invalid = report(m1, "shift-b", "2026-09-02T08:00:00Z", "2026-09-02T16:00:00Z", [
    metric("OEE", "calculated", { value: null }),
  ]);
...
test("whole boundary rejects out-of-order authority rather than reconstructing shift order", () => {
  const m1 = source("M1", "projection-m1", "Line A", 0, "line-a");
  const later = report(m1, "shift-b", "2026-09-02T08:00:00Z", "2026-09-02T16:00:00Z");
  const earlier = report(m1, "shift-a", "2026-09-02T00:00:00Z", "2026-09-02T08:00:00Z");
...
test("whole boundary rejects authority from outside the configured population", () => {
  const configured = source("M1", "projection-m1", "Line A", 0, "line-a");
  const foreign = source("M2", "projection-m2", "Line B", 1, "line-b");
  const foreignReport = report(foreign, "shift-a", "2026-09-02T00:00:00Z", "2026-09-02T08:00:00Z");

  expectFailure("unexpected-source", day, [configured], [foreignReport]);
});

test("whole boundary contains no current-state inference for configured machines without reports", () => {
  const m1 = source("M1", "projection-m1", "Line A", 0, "line-a");
  const machine = mapShiftPerformanceOverview(day, [m1], { items: [] }).groups[0].machines[0];
  ...
  assert.deepEqual(machine.shifts, []);
});
