...
public sealed class ProductionActivityProviderHandoffTests
{
    [Fact]
    public void SqlCurrentStateCompositionUsesProviderOwnedProductionReader()
    {
        ...
        var portableReader = provider.GetRequiredService<IProductionContextActivityReader>();

        Assert.NotNull(providerServices.MachineStateActivityAuthorityStore);
        Assert.NotNull(providerServices.ProductionContextActivityReader);
        Assert.Same(
        ...
        Assert.Same(observationGraph.ProductionActivityReader, association.ActivityReader);
        Assert.Same(association.ActivityReader, portableReader);
        Assert.IsNotType<JointProductionContextActivityReader>(portableReader);
    }

    [Fact]
    public void ProviderAuthorityWithoutMatchingProductionReaderFailsClosed()
    {
        ...
        var mappingStore = new InMemoryMappingCoverageAuthorityStore();
        var stateActivityStore = new InMemoryMachineStateActivityAuthorityStore();
        ...
        Assert.Contains(
            "without the matching production activity reader",
            exception.Message,
