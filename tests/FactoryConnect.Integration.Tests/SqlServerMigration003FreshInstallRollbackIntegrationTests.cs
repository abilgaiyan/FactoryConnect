...
public sealed class SqlServerMigration003FreshInstallRollbackIntegrationTests
{
    private static readonly string[] Migration001And002Tables =
    [
    ...
    [Fact]
    public async Task FreshInstallMigration003FailureRollsBackWholeInvocationAndRetryCompletesCurrentState()
    {
        ...
        await ExecuteAsync(
            connection,
            """
            CREATE TABLE dbo.C4FreshInstallConstraintConflict
            (
                Id int NOT NULL,
        ...
        foreach (var tableName in Migration001And002Tables)
        {
            Assert.False(await ObjectExistsAsync(connection, tableName));
        }
        ...
        Assert.True(await ObjectExistsAsync(connection, "dbo.C4FreshInstallConstraintConflict"));
        Assert.True(await ConstraintExistsOnTableAsync(
            connection,
            "UQ_MetricInputStream_RowMachine",
            "C4FreshInstallConstraintConflict"));

        await ExecuteAsync(connection, "DROP TABLE dbo.C4FreshInstallConstraintConflict;");
    ...
    private sealed class IsolatedMigrationDatabase : IAsyncDisposable
    {
        ...
        public static async Task<IsolatedMigrationDatabase> CreateAsync()
        {
            ...
            var databaseName = $"FactoryConnect_FC030_C4_Fresh_{Guid.NewGuid():N}";
